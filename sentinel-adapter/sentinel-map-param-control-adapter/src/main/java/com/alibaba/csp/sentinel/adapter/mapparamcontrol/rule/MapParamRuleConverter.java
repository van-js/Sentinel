 ...
 * @since 1.6.0
 */
final class MapParamRuleConverter {

    static FlowRule toFlowRule(/*@Valid*/ MapPramFlowRule rule) {
    ...
    static ParamFlowItem generateNonMatchPassParamItem() {
        return new ParamFlowItem().setClassType(String.class.getName())
            .setCount(1000_0000)
            .setObject(SentinelMapParamConstants.MAP_PARAM_NOT_MATCH_PARAM);
    }
    ...
    static ParamFlowRule applyNonParamToParamRule(/*@Valid*/ MapPramFlowRule gatewayRule, int idx) {
     ...
     * parameter index to {@link MapParamFlowItem} of the rule.
     *
     ...
     */
    static ParamFlowRule applyToParamRule(/*@Valid*/ MapPramFlowRule gatewayRule, int idx) {
        ParamFlowRule paramRule = new ParamFlowRule(gatewayRule.getResource())
            ...
            .setMaxQueueingTimeMs(gatewayRule.getMaxQueueingTimeoutMs())
            .setParamIdx(idx);
        MapParamFlowItem gatewayItem = gatewayRule.getParamItem();
